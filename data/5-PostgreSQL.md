...
# SQL (geometry)
Instant geometry type from literal representation:
```sql
select 'SRID=4326;POINT(115.7896 -31.9155)'::geometry;
```

Convert geometry to text:
```sql
select ST_AsText('SRID=4326;POINT(115.7896 -31.9155)'::geometry);
select ST_AsEWKT('SRID=4326;POINT(115.7896 -31.9155)'::geometry);
```